const ALGO = 'AES-GCM'; const ITERATIONS = 100000; const KEY_LENGTH = 256; const SALT_LENGTH = 16; const IV_LENGTH = 12;
export async function deriveKey(passphrase: string, salt: Uint8Array): Promise<CryptoKey> {
  const enc = new TextEncoder();
  const keyMaterial = await crypto.subtle.importKey('raw', enc.encode(passphrase), 'PBKDF2', false, ['deriveKey']);
  return crypto.subtle.deriveKey({ name: 'PBKDF2', salt, iterations: ITERATIONS, hash: 'SHA-256' }, keyMaterial, { name: ALGO, length: KEY_LENGTH }, false, ['encrypt', 'decrypt']);
}
...
export async function decryptData(encoded: string, passphrase: string): Promise<any> {
